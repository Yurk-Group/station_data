...
compute_dpt <- function(tib){ #computes drift potential from drift potential per time and time interval width
  tib <- tib %>% mutate(dpt = dp*dt)
  return(tib)
}

compute_dpt2 <- function(tib,dtmax=3){ #computes modified drift potentil by not allowing time intervals to exceed 3 hours. For those that do, the remainder of the time interval is recorded
  tib <- tib %>% mutate(dpt2 = dp*pmin(dt,3),lost_dt = dt-pmin(dt,3))
  return(tib)
}

compute_dp_stuff <- function(tib,dtmax = 3){ #applies the previous functions after dropping na wind speeds
...
compute_by_yr_stuff_directional <- function(tib){ #yearly totals: drift potential, by direction, by coast; lost time, number of observations, etc
...
drop_big_lost_dt_yrs<- function(tib,dtmax=175){ #drop years with more than 175 (default) hours of lost time
  tib <- tib[tib$lost_dt_su<=dtmax,]
  return(tib)
}


group_by_yrs <- function(tib,breaks){ #a function to group the data by years - used for decadal computations
  tib <- tib %>%
    mutate(num_yrs = 1) %>%
...
dp_plot_yrs <- function(tib,row,yscale=30,fname=NULL){ #plot and maybe save fryberger diagram for a particular row of the data
...
compute_rel_dp <- function(tib){ #Compute directional dps as proportion of total dp
