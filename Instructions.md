...
A list of stations and date ranges is given in the next section.
...
I wrote a separate script for cleaning/preprocessing the data from each station. I may have been able to reduce the amount of code duplication, but I did not spend the time to do it. Much of what I needed to do was particular to each station. These scripts are named as, for example, 'Combine_BEH.Rmd' for the BEH stations. These scripts are saved in the 'code' directory within the repository project directory.
...
**put table with summary information here or move to next section? Could print out at tope of analysis code, and not ever reproduce it here (probably the best). If so, then change reference above**
